...
### 1.2 Задание символьной ссылки до исполняемого файла OpenSSL

```
ln -s $(whereis openssl | awk '/bin/ {print $2}') ./openssl
```
...
Если исполняемый файл отсутствует в системе, то требуется его устеновить командой
...
./do.sh enroll
```
где **2-ой** входной параметр представляет на выбор 3 профиля сертификатов:
- `client`, TLS-клиента
- `server`, TLS-сервера

где **3-ий** входной параметр представляет возможность выпустить сертификат:
- `by-request`, по запросу пользователя
  - последующее нажатие **TAB** отобразит список доступных запросов на издание
...
- `one-key-file-gen`, с генерацией программного личного ключа, размещаемого в одном файле. Если на данном этапе отбить команду, то пойдет генерация личного ключа в открытом виде
  - профили по умочанию для запросов ( client, server и entity_person ) на сертификат определены в `./profiles/default-users/`, которые можно переопределить
...
```
1) Выпуск сертификата с генерацией личного ключа, разбиваемого на частичные секреты
```
./do.sh enroll server gen-key-divided-secrets
```
...
## 2.5 Издание списка отозванных сертификатов ( СОС )
...
После процедуры отзыва сертификатов происхода обновление CRL на всех FTP-серверах, так же есть возможность отправить сертификаты УЦ ( Корневого и Подчиненного ) на список FTP-серверов, которые требуется определить в ассоциативном массиве `ftp_servers` файла `publish-by-ftp.hosts.sh`
...
├── divided-secrets-generator       # Программа по генерации л. ключа с частичными секретами и запроса на сертификат
...
#### 3.2.3 База данных серийных номеров списка отозванных сертификатов ( CRL )
